...
if (!fs.existsSync(specPath)) {
  throw new Error(
    'OPENAPI spec not found at ' +
      specPath +
      '. Run `cargo build -p ozzy-api` (or set OPENAPI_SPEC_PATH) before building the SDK.'
  );
}
...
export class OzzyNodeSdkClient {
  #baseUrl;
  #apiKey;
  #fetch;
  #defaultHeaders;
...
const indexTypes = `export interface OzzyNodeSdkOptions {
  baseUrl?: string;
  apiKey?: string;
...
export type OzzyNodeSdkEndpoint = {
  method: string;
  path: string;
  operationId: string;
  summary?: string;
...
export const openapiEndpoints: OzzyNodeSdkEndpoint[];

export class OzzyNodeSdkClient {
  constructor(options?: OzzyNodeSdkOptions);
...
console.log(`built ozzy-node-sdk from ${specPath}`);
