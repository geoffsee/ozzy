...
oz may send **anonymous usage telemetry** from the Web UI and CLI to help understand how the software is used and where it fails. Telemetry is optional for self-hosted deployments and can be disabled entirely.

Telemetry is collected through [g-telemetry](https://github.com/geoffsee/g-telemetry), a self-hosted analytics sink. The official oz deployment sends events to a dedicated ingestion worker; you control whether telemetry is enabled on your own instance via `TELEMETRY_SINK_URL` in `wrangler.toml` (see [Wrangler configuration](wrangler.md)).
...
Do not put personally identifiable information in telemetry `properties`. oz does not intentionally send PII, but misconfiguration of a custom sink is your responsibility.
...
| CLI | `TELEMETRY_SINK_URL` or `OZ_TELEMETRY_ENDPOINT` is set in the environment | Variables unset |
...
2. Set an app-specific override:
   - Web UI: `OZ_WEB_NO_TELEMETRY=1`
   - CLI: `OZ_CLI_NO_TELEMETRY=1`
...
Events are POSTed to `{TELEMETRY_SINK_URL}/v1/events` and stored in the operator’s g-telemetry deployment (Cloudflare Worker + D1 by default). Access to aggregate reporting endpoints on that sink is restricted to the operator; oz itself does not expose telemetry data through the secrets API.

If you operate your own oz instance, you choose whether to enable telemetry and where the sink lives. If you use someone else’s hosted oz deployment, their privacy policy and telemetry configuration apply.

## Questions

For issues with the telemetry clients or sink, see the [g-telemetry repository](https://github.com/geoffsee/g-telemetry). For oz-specific behavior, open an issue on the [oz repository](https://github.com/geoffsee/oz).
