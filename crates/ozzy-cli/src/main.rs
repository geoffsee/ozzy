...
use ozzy_core::{Project, SecretMeta, SecretValue};
...
#[derive(Parser)]
#[command(name = "ozzy", about = "ozzy secrets CLI")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
fn handle_auth(command: AuthCommands) -> Result<()> {
    match command {
        AuthCommands::Login { api_key, api_url } => {
            ...
            if let Some(key) = api_key.or_else(|| std::env::var("OZZY_API_KEY").ok()) {
                cfg.api_key = Some(key);
            } else if cfg.api_key.is_none() {
                bail!("provide --api-key or set OZZY_API_KEY");
            }
            cfg.save()?;
