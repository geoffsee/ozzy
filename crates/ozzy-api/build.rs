...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=OZZY_TELEMETRY_ENDPOINT");
    ...
    let status = build_cmd
        .status()
        .expect("failed to execute bun build for ozzy web client");

    if !status.success() {
        panic!("bun web build failed while compiling ozzy-api");
    }
    ...
    println!("cargo:rustc-env=OZZY_UI_HTML_PATH={}", output_file.display());
    ...
    println!(
        "cargo:rerun-if-changed={}",
        workspace_root.join("crates/ozzy-core/src/models.rs").display()
    );
    println!(
        "cargo:rerun-if-changed={}",
        workspace_root.join("crates/ozzy-core/src/lib.rs").display()
    );
}
...
fn resolve_telemetry_events_endpoint() -> Option<String> {
    env::var("BUN_PUBLIC_TELEMETRY_ENDPOINT")
        ...
        .or_else(|| {
            env::var("OZZY_TELEMETRY_ENDPOINT")
                .ok()
                .filter(|value| !value.is_empty())
        })
}
